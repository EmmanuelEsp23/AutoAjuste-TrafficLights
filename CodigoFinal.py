...
# Valores predeterminados para los tiempos de los semáforos
...
# Parámetros para el tiempo verde dinámico
T_min = 5
T_max = 30

def calcularTiempoVerde(direction):
    
    # 1. Contar vehículos esperando en la dirección dada
    vehiculosEnDir = 0
    for lane in range(3): 
         vehiculosEnDir += sum(1 for v in vehicles[direction][lane] if v.crossed == 0)

    if vehiculosEnDir == 0:
        return T_min  # Mínimo si no hay nadie

    # 2. Definir un umbral máximo de vehículos para espera
    VEHICULOS_MAX_ESPERA = 45 

    # 3. Calcular la proporción de congestión
    congestion_ratio = min(vehiculosEnDir / VEHICULOS_MAX_ESPERA, 1.0)

    # 4. Asignar tiempo basado en esa congestión
    tiempo = T_min + int(congestion_ratio * (T_max - T_min))
...
def repeat():
    ...
    # Ajustar dinámicamente el tiempo verde según congestión
    dir_actual = directionNumbers[currentGreen]
    signals[currentGreen].green = calcularTiempoVerde(dir_actual)
    ...
    currentYellow = 1
    for i in range(0,3):
        for vehicle in vehicles[dir_actual][i]:
            vehicle.stop = defaultStop[dir_actual]
